...
### PATCH `/lectures/{lectureId}` 修改讲座信息，不用带上全部数据
...
```json
{
    ...
    "data": {
        ...
        "status": "runing/ended/prepare", //进行中、已结束、准备中
...
```json
{
    "status":"",//ended,siging,stopsiging 结束，开始签到，结束签到
}
```
...
### GET `/lectures/{lectureId}/status` 获取讲座状态
...
```json
{
    ...
    "msg":"ok",
    "data":{
        "status":"",//状态 prepare, ended, siging, stopsiging 准备中, 结束，开始签到，结束签到
    }
}
```
